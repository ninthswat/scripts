...
if ! systemctl is-active --quiet httpd; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') - HTTPD сервер недоступен" >> "$LOG_FILE"
    send_alert "ВНИМАНИЕ: HTTPD не работает" "Сервер HTTPD (Apache) не запущен. Пытаюсь перезапустить..."
    
    systemctl restart httpd
    sleep 5
    
    if systemctl is-active --quiet httpd; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') - HTTPD успешно перезапущен" >> "$LOG_FILE"
        send_alert "ВОССТАНОВЛЕНО: HTTPD работает" "Сервер HTTPD был успешно перезапущен"
    else
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Ошибка перезапуска HTTPD!" >> "$LOG_FILE"
        send_alert "КРИТИЧЕСКО: Ошибка перезапуска HTTPD" "Не удалось перезапустить сервер HTTPD! Требуется ручное вмешательство."
    fi
fi
